 ...
 */
import { CHARSET_MAP, CharsetPreset } from '../types/interfaces.js';
import { extractPixelData, calculateLuminance, luminanceToChar, calculateDimensions, renderTextToCanvas, samplePixelColor, rgbToCSS } from '../utils/canvas-helpers.js';
/**
...
export class AsciiGenerator {
    ...
    convertImage(source) {
        ...
        const dimensions = calculateDimensions(pixelData.width, pixelData.height, this.config.width || undefined, this.config.height || undefined, this.config.aspectRatio);
        ...
        const resizedPixelData = extractPixelData(source, dimensions.width, dimensions.height);
    ...
    convertText(text, options = {}) {
        const { font = 'Arial', fontSize = 48, fontWeight = 'normal', fontStyle = 'normal', color = '#000000', backgroundColor = '#ffffff', padding = 10 } = options;
        // Render text to canvas
        const imageData = renderTextToCanvas(text, font, fontSize, fontWeight, fontStyle, color, backgroundColor, padding);
    ...
    generateColorMap(source) {
        ...
        for (let y = 0; y < dimensions.height; y++) {
            ...
            for (let x = 0; x < dimensions.width; x++) {
                const color = samplePixelColor(resizedPixelData, x, y, dimensions.width, dimensions.height);
                row.push(color);
            }
            colorMap.push(row);
        }
    ...
    updateConfig(config) {
        ...
        if (config.inverted !== undefined)
            this.config.inverted = config.inverted;
        if (config.colored !== undefined)
            this.config.colored = config.colored;
        ...
        if (config.optimized !== undefined)
            this.config.optimized = config.optimized;
        this.validateConfig();
    }
    /**
     ...
     */
    processPixelData(pixelData, dimensions) {
        const { width, height } = dimensions;
        const characters = [];
        const colors = this.config.colored ? [] : undefined;
        const lines = [];
        const htmlLines = [];
        ...
        if (this.config.optimized) {
            ...
            if (colors)
                colors.length = height;
        }
        for (let y = 0; y < height; y++) {
            const charRow = this.config.optimized ? new Array(width) : [];
    ...
    resolveCharset(charset) {
        if (typeof charset === 'string' && Object.values(CharsetPreset).includes(charset)) {
